...
response = client.send(pp.GetCpmToken(
                          "mQU81piDFRyBs61QA64ubF"                              # cpm_token: CPMトークン
))
```
...
response = client.send(pp.ListTransactions(
                          start="2024-10-11T03:05:13.000000Z",                  # 開始日時
...
response = client.send(pp.CreateTransaction(
                          ...
                          money_amount=4629,
                          point_amount=2543,
                          point_expires_at="2021-01-11T07:10:34.000000Z",       # ポイント有効期限
...
response = client.send(pp.ListTransactionsV2(
                          ...
                          customer_name="太郎",                                   # エンドユーザー名
                          description="店頭QRコードによる支払い",                          # 取引説明文
                          transaction_id="8KqA",                                # 取引ID
                          is_modified=False,                                    # キャンセルフラグ
                          types=["topup", "payment"],                           # 取引種別 (複数指定可)、チャージ=topup、支払い=payment
                          start="2022-12-28T12:10:25.000000Z",                  # 開始日時
...
response = client.send(pp.ListBillTransactions(
                          ...
                          is_modified=True,                                     # キャンセルフラグ
                          start="2022-03-10T10:51:09.000000Z",                  # 開始日時
...
response = client.send(pp.CreateTopupTransaction(
                          ...
                          money_amount=442,                                     # マネー額
                          point_amount=5099,                                    # ポイント額
                          point_expires_at="2023-08-21T15:11:02.000000Z",       # ポイント有効期限
...
response = client.send(pp.CreateCpmTransaction(
                          "rtqclFU9jljopVrQrbVbWU",                             # cpm_token: CPMトークン
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # shop_id: 店舗ID
                          5874.0,                                               # amount: 取引金額
...
response = client.send(pp.RefundTransaction(
                          ...
                          returning_point_expires_at="2022-12-21T18:37:32.000000Z" # 返却ポイントの有効期限
))
```
...
```PYTHON
response = client.send(pp.CancelBulkTransaction(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"                # bulk_transaction_id: バルク取引ジョブID
))
```
...
```PYTHON
response = client.send(pp.ResumeBulkTransaction(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"                # bulk_transaction_id: バルク取引ジョブID
))
```
