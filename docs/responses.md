...
* `name2 (str)`: 顧客名
* `amount (int)`: チャージ額
* `service_option_type (str)`: コンビニ種別
* `haraikomi_url (str)`: 払込票URL
* `receipt_no (str)`: 受付番号
* `done_at (str)`: 入金完了日時
* `canceled_at (str)`: キャンセル日時
...
* `per_page (int)`: 
* `count (int)`: 
* `items (list of CvsAuthorizations)`: 
* `prev (str)`: 
* `next (str)`: 

`items`は [CvsAuthorization](#cvs-authorization) オブジェクトのリストを返します。
...
## CardAuthorizeResult
* `authentication_html (str)`: 認証開始用HTML
...
* `additional_private_moneys (list of PrivateMoneys)`: 追加の支払いマネー

`account`は [AccountWithUser](#account-with-user) オブジェクトを返します。

`additional-private-moneys`は [PrivateMoney](#private-money) オブジェクトのリストを返します。
...
* `applicable_shop_labels (list of CampaignShopLabels)`: キャンペーン適用対象の店舗ラベル指定
...
`applicable-shop-labels`は [CampaignShopLabel](#campaign-shop-label) オブジェクトのリストを返します。
